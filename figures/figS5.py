...
data_files = ['t11_231024_008_timing.csv', 't11_231024_007_timing.csv']
timing_dfs = []
for data_file in data_files:
    data_path = os.path.join(DATA_DIR, data_file)
    with open(data_path, 'r') as f:
        timing_df = pd.read_csv(f)
...
for i, timing_df in enumerate(timing_dfs):
    axr = axes[i, :]
    all_ts_fields = {}
    for i in range(2):
        ts_fields = [
        ...
        if 'ts_lfads' not in timing_df.columns:
            ts_fields.remove('ts_lfads')
...
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, 'figS5.pdf'), dpi=300)
plt.show()
